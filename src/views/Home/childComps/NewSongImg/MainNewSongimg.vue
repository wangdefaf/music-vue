<template>
  <div class="newsong">
    <home-template :lists="tagTile" routerName="Album" title="新碟首发" @switchIndex="switchIndex"/>
...
export default {
  ...
  methods: {
    ...
    getNewSongFun() {
      ...
      getNewSong(tag, 20).then(res => {
        this.goods = new Array(Math.floor(res.albums.length / 10))
        for (let i = 0; i < this.goods.length; i++) {
          this.goods[i] = res.albums.splice(0, 10)
        }
